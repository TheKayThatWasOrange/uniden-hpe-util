...
Need to add thousands and thousands of custom channels which no unemployable boomer in the world has ever cared enough about to post on some radio dork forum for radio dorks? Well you get to sit there and create them yourself one-by-friggin-one because Uniden hates people who have jobs--they aren't even cool enough to know how to use a catheter as a ground plane. You might be tempted to think that you can just clone an existing channel over and over and then type in the new frequency to speed through all that data entry tedium the way you might in an actual spreadsheet, but you'd be living in some kind of Bizarro world where Uniden hires competent product managers. Sentinel is here to guard you from socialist constructs like convenience and ease of use. What are you? Some kind of ANTIFA snowflake?
...
Luckily, [this person](https://github.com/sq5bpf/hpe_open) found out that `.hpe` files are nothing more than XORed GZip files which can be converted to/from plain text pretty easily and Google had kept a record of that fact. Unluckily, his makefile doesn't work anywhere so I just translated the basic process into reasonably modern Python. I say that having written zero Python in the last two years so my idea of "reasonably modern" may be "utterly laughable." Why do I think it's modern? Because I used the walrus operator. Twice!
